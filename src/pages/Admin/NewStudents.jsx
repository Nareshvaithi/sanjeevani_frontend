...
import {
  deleteNewStudent,
  selectNewStudents,
} from "../../store/adminSlices/newStudentSlice";
      ...
      <div className="bg-white px-3 py-5 rounded-lg">
        ...
        <div className="w-full overflow-x-auto">
          {newStudentList.length === 0 ? (
            <p className="text-sm">No Waiting Students Available</p>
          ) : (
            <table className="min-w-max w-full border-collapse text-left">
                ...
                <tr>
                  {studentDataTitle.map(({ id, title }) => (
                    <th
                      key={id}
                      className={`${
                        title === "Batch" || title === "Status" ? "hidden" : ""
                      } border-b border-gray-300 py-4 px-2 whitespace-nowrap`}
                    >
                      {title}
                    </th>
                  ))}
              ...
              <tbody>
                {newStudentList && newStudentList.length > 0 ? [...newStudentList]
                  ?.reverse()
                  .slice(0, 11)
                  .map((student) => {
                    const {
                    ...
                    return (
                      <tr key={_id} className="odd:bg-gray-100">
                        ...
                        <td className="py-4 px-2 whitespace-nowrap">
                          <span
                            className={`${
                              findPayStatus ? "bg-green-500" : "bg-red-500"
                            } text-[12px] text-white px-2 py-1 rounded-md`}
                          >
